...
class AssociationRecommender(BaseRecommender):
    def recommend(self, dataset: Dataset, **kwargs) -> RecommendResult:
        ...
        # 支持度が高い映画
        freq_movies = apriori(
            user_movie_matrix.astype("bool"), min_support=min_support, use_colnames=True
        )
        # アソシエーションルールの計算（リフト値の高い順に表示）
        rules = association_rules(
            freq_movies, metric="lift", min_threshold=min_threshold
        )
        ...
        for user_id, data in movielens_train_high_rating.groupby("user_id"):
            ...
            matched_flags = (
                rules.antecedents.apply(lambda x: len(set(input_data) & x)) >= 1
            )

            # アソシエーションルールの帰結部の映画をリストに格納し、登場頻度順に並び替え、ユーザーがまだに評価していないければ、推薦リストに追加する
            consequent_movies = []
            for i, row in (
                rules[matched_flags].sort_values("lift", ascending=False).iterrows()
            ):
